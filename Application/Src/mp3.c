...
uint32_t FillMp3Buf(uint8_t *Buf)
{
	...
	Offset=MP3FindSyncWord(Readptr,ByteLeft);	//��readptrλ��,��ʼ����ͬ���ַ�
	...
	ByteLeft-=Offset;													//buffer�������Ч���ݸ���,�����ȥƫ����
	//printf("ByteLeft=%d\r\n",ByteLeft);
	if(ByteLeft<MAINBUF_SIZE*2)//����������С��2��MAINBUF_SIZE��ʱ��,���벹���µ����ݽ���.
	{ 
		memmove(TempBuf,Readptr,ByteLeft);//�ƶ�readptr��ָ������ݵ�buffer����,��������СΪ:bytesleft
		/*
		ReadBytesNum=WAVEFILEBUFSIZE-ByteLeft;
		if(ReadBytesNum%2)
			ReadBytesNum-=1;
		f_read(&Mp3File,TempBuf+ByteLeft,ReadBytesNum,&br);//�������µ�����
		printf("rd=%d,br=%d\r\n",ReadBytesNum,br);
		if(br<ReadBytesNum)
		{
		...
		*/
		f_read(&Mp3File,TempBuf+ByteLeft,WAVEFILEBUFSIZE-ByteLeft,&br);//�������µ�����
	...
	err=MP3Decode(Mp3Decoder,&Readptr,&ByteLeft,(int16_t*)Mp3DecodeBuf,0);//����һ֡MP3����
